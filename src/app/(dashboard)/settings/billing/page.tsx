...
import { Loader2, CreditCard, CheckCircle2, ArrowUp, ArrowDown, X } from "lucide-react";
...
const plans = [
    ...
    },
    {
        ...
        features: ["50 resumes/month", "3 recruiters", "Full dashboard", "Basic automations"],
    },
    {
        id: "pro",
        name: "Pro",
...
export default function BillingSettingsPage() {
    ...
    const [modalAction, setModalAction] = useState<"upgrade" | "downgrade" | "cancel">("upgrade");
        ...
        const fetchProfile = async () => {
            try {
                ...
                if (res.ok) {
                    ...
                    setCurrentPlan(data.user.plan || "free_trial");
                }
            } catch (error) {
    ...
    const handlePlanAction = (planId: string, action: "upgrade" | "downgrade" | "cancel") => {
    ...
    const confirmPlanChange = async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                toast({
                    title: "Plan Updated",
                    description: `Your plan has been changed. Note: This is a demo—no actual billing occurred.`,
                    variant: "success",
                });
            } else {
            ...
            <Card className="bg-amber-500/10 border-amber-500/30">
                <CardContent className="py-4">
                    <p className="text-sm text-amber-700 dark:text-amber-400">
                        <strong>Demo Notice:</strong> This is a demonstration. Billing is mocked.
                        Stripe/Razorpay integration will be added later via n8n webhooks.
                    </p>
                </CardContent>
            </Card>
            ...
            {currentPlan !== "free_trial" && (
                <Card className="border-red-500/30">
                    <CardHeader>
                        ...
                        <CardDescription>
                            Downgrade to the free trial. You&apos;ll lose access to premium features.
                        </CardDescription>
                    </CardHeader>
                    <CardFooter>
                        <Button
                        ...
                        <DialogDescription>
                            <div className="mt-4 p-4 rounded-lg bg-[hsl(var(--muted))]">
                                <p className="text-sm mb-2">
                                    You are about to{" "}
                                    {modalAction === "upgrade" ? "upgrade to" : modalAction === "downgrade" ? "downgrade to" : "cancel and switch to"}{" "}
                                    <strong>{plans.find((p) => p.id === selectedPlan)?.name}</strong>.
                                </p>
                                <p className="text-sm text-[hsl(var(--muted-foreground))]">
                                    In this demo, billing is mocked. Connect Stripe/Razorpay later via n8n webhooks.
                                </p>
                            </div>
                        </DialogDescription>
                    </DialogHeader>
